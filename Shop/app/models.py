...
class Goods(db.Model):
    ...
    collect = db.relationship("Collect", backref='goods')  # 订单外键关系关联
...
class Regions(db.Model):
    ...
    RegionManager = db.Column(db.String(255))

    def __repr__(self):
        return "<Regions %r>" % self.RegionName
...
class Customers(db.Model):
    ...
    BusinessCategory = db.Column(db.String(255))
    AnnualRevenue = db.Column(db.DECIMAL(10, 2))
    MaritalStatus = db.Column(db.String(255))
    Gender = db.Column(db.String(255))
    Age = db.Column(db.Integer)
    PersonalIncome = db.Column(db.DECIMAL(10, 2))

    def __repr__(self):
        return "<Customers %r>" % self.Name

# 销售品牌
class SalesBrand(db.Model):
    __tablename__ = "salesbrand"
    ...
    income = db.Column(db.DECIMAL(10, 2))

    def __repr__(self):
        return "<SalesBrand %r>" % self.name